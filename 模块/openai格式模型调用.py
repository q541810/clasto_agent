...
async def 调用模型(模型名,user提示词,system提示词,额外参数=None): # 定义异步模型调用包装方法
    # 遍历模型配置二维数组，寻找匹配的模型名称
    for 单个模型 in 模型配置:
        if 单个模型[0] == 模型名:
            模型url = 单个模型[3] # 对应 reading_config.py 中的 url 索引
            模型id = 单个模型[2]  # 对应 reading_config.py 中的 模型id 索引
            api_key = 单个模型[4] # 对应 reading_config.py 中的 api_key 索引
            # 使用 await 调用基础方法
